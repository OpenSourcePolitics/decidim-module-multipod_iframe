      ...
      describe "iframe_src" do
        ...
        context "when there is no query_params added in the iframe url" do
          ...
          it "returns the url with similarto as query_params" do
            expect(helper.iframe_src(proposal)).to eq("https://api.example.org?similarto=#{CGI.escape(string)}")
          end
        end

        context "when there is a query_params added in the iframe url" do
          before do
            component.update!(settings: { enable_iframe: true, iframe_url: "https://api.example.org?date=2026-01-01,2026-04-30" })
          end

          it "adds similarto to the query_params list" do
            expect(helper.iframe_src(proposal)).to eq("https://api.example.org?date=2026-01-01,2026-04-30&similarto=#{CGI.escape(string)}")
          end
        end
      end
    end
